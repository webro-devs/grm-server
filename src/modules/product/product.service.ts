      ...
      querybuilder.andWhere(
        new Brackets((cb) => {
          cb.where('product.count > 0')
            .where('LOWER(product.shape) LIKE LOWER(:search)', { search: `%${where['search']}%` })
            .where('LOWER(collection.title) LIKE LOWER(:search)', { search: `%${where['search']}%` })
            .where('LOWER(product.size) LIKE LOWER(:search)', { search: `%${where['search']}%` })
            .where('LOWER(model.title) LIKE LOWER(:search)', { search: `%${where['search']}%` })
            .where('LOWER(product.style) LIKE LOWER(:search)', { search: `%${where['search']}%` });
        }),
      );
      querybuilder
